    ...
    <g:form controller="visit" action="update">
        ...
        <div class="form-group">
            ...
            <g:select name="pet.id" from="${pets}" optionKey="id"
                      ...
                      value="${this.visit?.pet?.id}" required="" class="form-control"/>
        </div>
        <div class="form-group">
            <label for="date">Date</label>
            <g:datePicker name="date" value="${this.visit?.visitDate}" required="" class="form-control"/>
        </div>
        <div class="form-group">
            <label for="description">Description</label>
            <g:textField name="description" value="${this.visit?.description}" required="" class="form-control" pattern="[A-Za-z0-9 .,!?-]+" inputmode="text" title="Only letters, numbers, spaces, and . , ! ? - are allowed."/>
        </div>
