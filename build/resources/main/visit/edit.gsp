...
<div class="card mb-4 ${params?.petId != null ? 'col-md-6' : ''} ">
    ...
    <div class="card-body">
        ...
        <g:form controller="visit" action="update">
            ...
            <div class="row gx-3 mb-3">
                <div class="col-md-6 ${params?.petId != null ? 'd-none' : ''} ">
                    <label class="small mb-1" for="inputVisitPet">Pet</label>
                    <g:select id="inputVisitPet" name="pet.id" from="${pets}" optionKey="id" optionValue="${(pet) -> "${pet.name} (${pet.type})"}" value="${params?.petId ?: this.visit?.pet?.id}" required="" class="form-control"/>
                </div>
                <div class="col-md-${params?.petId != null ? '12' : '6'}">
            ...
            <g:textField name="petId" value="${params?.petId}" hidden="hidden" />

            <!-- Return to Owner Details if param is available -->
            <g:textField name="fromShow" value="${params?.fromShow ?: null}" hidden="hidden" />
